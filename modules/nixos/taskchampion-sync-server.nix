...
in
{
  options.services.taskchampion-sync-server = {
    ...
    enableTraefik = lib.mkEnableOption "Bind port to subdomain taks through traefik reverse-proxy";
  ...
  config = lib.mkIf cfg.enable {
    ...
    services.traefik.dynamicConfigOptions = lib.mkIf (cfg.enableTraefik) {
      ...
      services = {
        taskchampion.loadBalancer.servers = [ { url = "http://127.0.0.1:${cfg.port}"; } ];
      };
    };
